...
async function testAgentProtocolsAndFixtures() {
  ...
  assert(implementWorkflow.includes('Maximum five simultaneous implementation agents'), 'implementation wave is not capped at five');
  ...
  assert(implementWorkflow.includes('project-context ready --limit 5'), 'implementation workflow does not use the ready-wave scheduler');
