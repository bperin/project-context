...
research subagent for primary sources, then a reviewer subagent for
review. The reviewer gets context (a summary + the research file), not
...
Orchestrator loads adhd (divergent ideation on the design space)
    ...
    → Orchestrator dispatches reviewer subagent (spec-optimizer, with context)
...
primary sources. The reviewer has context (a summary + the research
file) and challenges scope, coverage, and requirements.
...
### Role 3: Reviewer (spec-optimizer subagent, read-only, with context)
...
4. **Dispatch the reviewer** (foreground, `spec-optimizer` profile —
   ...
   problem fit from divergent angles before reviewing. Give it:
   - The spec file path
...
5. **Apply reviewer findings.** Revise the spec. If the reviewer found
   a fundamental problem (wrong problem, wrong scope), stop and discuss
   with the user before rewriting.

6. **Round counter.** If the reviewer found MUST-FIX or SHOULD-FIX
   issues, go back to step 4 (re-dispatch the reviewer on the revised
   spec). Max 3 rounds. If still no agreement after round 3, escalate
   to the user with a summary of the disagreement.

7. **Commit.** When the reviewer passes (only NITs or clean), commit
   the spec with a message summarizing what the review changed.
