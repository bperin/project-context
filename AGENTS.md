...
- **Parallel analysis, serial mutation.** During task creation, up to four pinned read-only workstream analysts may run in parallel. Writers, reviewers, implementation, and all state mutation remain sequential. One implementation task at a time.
